  ...
  const fetchCurrentLocation = (): Promise<{lat: number, lng: number}> => {
    return new Promise((resolve) => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.warn("Chat Assistant GPS failed, using grid baseline.", err);
          resolve({ lat: 30.0668, lng: 79.0193 });
        },
  ...
  const executeReportAction = async (description: string, lat: number, lng: number) => {
    ...
    try {
      ...
      const successMsg: Message = {
        ...
        role: 'assistant',
        content: `Tactical report submitted successfully. Ground teams alerted.`,
        timestamp: new Date(),
        isAction: true
      };
      setMessages(prev => [...prev, successMsg]);
      toast({ title: "Grid Updated", description: "Autonomous interdiction report submitted." });
      
    } catch (error) {
      console.error("Action error:", error);
      toast({ variant: "destructive", title: "Interdiction Failed", description: "Could not sync report with grid." });
    } finally {
    ...
    try {
      ...
      if (result.redirectTo) {
        ...
        setTimeout(() => {
          ...
          toast({ title: "Interface Switch", description: `Navigating to ${result.redirectTo}` });
        }, 1200);
      }
      ...
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: "I am having difficulty accessing the AI grid. Please try text input.",
        timestamp: new Date(),
      }]);
    } finally {
      setIsThinking(false);
    ...
    <>
      <div className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-[100]">
        <button
          ...
          className={cn(
            "w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center transition-all duration-500 shadow-2xl hover:scale-110 border border-white/10",
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-4 left-4 md:left-auto md:bottom-24 md:right-8 w-auto md:w-[360px] h-[70vh] md:h-[540px] z-[90] flex flex-col liquid-glass rounded-[2.5rem] border-white/5 shadow-2xl animate-in slide-in-from-bottom-10 fade-in duration-500 overflow-hidden">
          <header className="p-5 border-b border-white/5 flex items-center justify-between shrink-0 bg-black/20">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20">
                <Sparkles className="w-4 h-4 text-primary animate-pulse" />
              </div>
              <div className="text-left">
                <h3 className="text-white font-headline font-bold text-xs tracking-tight leading-none uppercase">{t('assistant.name')}</h3>
                <p className="text-[6px] text-primary font-bold uppercase tracking-[0.3em] mt-1">{t('assistant.status')}</p>
              </div>
            </div>
            <button onClick={clearChat} title="Purge Log" className="text-white/20 hover:text-destructive transition-colors p-2">
              ...
              {messages.length === 0 && (
                <div className="text-center py-20 space-y-4 opacity-20">
                  ...
                  <p className="text-[9px] font-bold uppercase tracking-[0.2em] max-w-[200px] mx-auto leading-relaxed">
                    Tactical Link Established. Status: Monitoring.
                  </p>
                </div>
              )}
              {messages.map((msg) => (
                <div key={msg.id} className={cn(
                  ...
                  <div className={cn(
                    ...
                    {msg.isAction && (
                      <div className="mt-3 flex items-center gap-2 text-primary font-bold animate-pulse">
                        <ShieldCheck className="w-3.5 h-3.5" />
                        <span className="text-[8px] uppercase tracking-widest">SITREP LOGGED</span>
                      </div>
                    )}

                    {msg.redirectTo && (
                      <div className="mt-3 p-2 bg-white/5 rounded-xl border border-white/10 flex items-center justify-between">
                        <span className="text-[8px] font-bold text-primary uppercase">Focusing Interface...</span>
                  ...
                  <span className="text-[6px] text-white/20 font-bold uppercase tracking-widest px-1">
                    {msg.role === 'user' ? 'GND' : 'TAC'} • {msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
              ))}
              {isThinking && (
                <div className="flex items-start gap-2">
                  <div className="bg-white/5 p-4 rounded-2xl rounded-tl-none flex items-center gap-3 border border-white/10">
                    ...
                    <span className="text-[8px] font-bold text-primary uppercase tracking-widest animate-pulse">Synchronizing...</span>
                  </div>
                </div>
              )}
              {isExecuting && (
                <div className="flex flex-col items-center gap-3 w-full py-6 animate-in fade-in zoom-in-95">
                  ...
                  <span className="text-[8px] font-bold text-primary uppercase tracking-[0.3em] animate-pulse text-center">Interdicting Grid...</span>
                </div>
              )}
            </div>
          </ScrollArea>
          ...
          <form onSubmit={handleSend} className="p-5 bg-black/40 backdrop-blur-3xl border-t border-white/5 shrink-0">
            <div className="relative group">
              ...
              <button 
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center disabled:opacity-30 disabled:grayscale transition-all hover:scale-105 active:scale-95 shadow-lg"
              >
